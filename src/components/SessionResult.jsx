...
export default function SessionResult({
  ...
  const [transferTo, setTransferTo] =
    useState(participants[0] || "");
  const [
    settledPlayers,
    setSettledPlayers,
  ] = useState(
    session.settledPlayers || [],
  );
  ...
  useEffect(() => {
    ...
    if (
      !transferTo ||
      !participants.includes(transferTo)
    ) {
  ...
  const handleToggleSettled = (
    ...
    const nextSettled =
      settledPlayers.includes(name)
        ? settledPlayers.filter(
            (player) => player !== name,
          )
        : [...settledPlayers, name];
                  ...
                  (entry) => {
                    ...
                    return (
                      <tr key={entry.id}>
                        <td>
                          <span
                            className={`type-badge ${entry.type}`}
                          >
                            {getEntryLabel(
                              entry,
                            )}
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {canSettle ? (
                        <label
                          className="settle-toggle"
                          aria-label={`Đánh dấu ${name} đã thanh toán`}
                        >
                          <input
                            type="checkbox"
